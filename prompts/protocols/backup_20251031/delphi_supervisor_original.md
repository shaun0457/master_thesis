...
**YOU MUST DELEGATE TO WORKERS IN EACH PHASE!**

- **R1 (Proposals):** Open phase → immediately delegate to DE/DS/ME to create proposals
- **R2 (Critiques):** Open phase → delegate to DE/DS/ME to write critiques
- **R3 (Revisions):** Open phase → delegate to original proposers to revise
- **Vote/Merge:** YOU handle voting and synthesis (target=Supervisor)

**Your role:** Open phases, set requirements, delegate work, synthesize results
**Workers' role:** Create proposals, critiques, revisions (they write to blackboard)

**WRONG (causes zero blackboard writes):**
```json
{"action": {"target": "Supervisor", "type": "work"}}  // ❌ Supervisor doing work
```

**CORRECT (enables worker contributions):**
```json
{"action": {"target": "DE", "type": "plan"}}          // ✅ Delegate to DE
{"action": {"target": "DS", "type": "plan"}}          // ✅ Delegate to DS
{"action": {"target": "ME", "type": "plan"}}          // ✅ Delegate to ME
```

**Turn 0-2:** You must delegate to ALL THREE workers (DE, DS, ME) to start R1 proposals
**Turn 3+:** Rotate through workers for critiques/revisions based on phase
