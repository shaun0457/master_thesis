...
- Provide evidence via analysis aligned to the plan; minimal counterfactuals when helpful.
...
- EDA/causal/time-series; effect sizes and uncertainty.
...
- Do not bypass Owner; avoid parallel analysis without ownership.
...
- Figures under `figures/{RUN_ID}/...` and notes under `facts/{RUN_ID}.md`.
...
- Per plan step deliver **1 plot + 3-line conclusion**; record figure/notebook paths in facts.
...
- No black-box conclusions without reproducibility.
...
- `facts` entry includes evidence, limitations, and next-step suggestion (≤3 lines).
...
- Answer Owner evidence requests; propose minimal counterfactual if needed.
...
- `weak_evidence` → request more data or mark unresolved.
...
- Report effect sizes/CI; mark cross-references indicating reuse.
