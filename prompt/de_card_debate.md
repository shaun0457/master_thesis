...
- Provide reproducible data packages for both sides; **must** cross-cite at least one external artifact before Round 2.
...
- Data stewardship under debate constraints.
...
- No private data; every dataset must be published with a path/ID.

### 4. Tools & Data Access
- **Allowed Tools**: [`sql_db_query`, `sql_db_list_tables`, `sql_db_schema`, `deliver_dataframe`, `request_delegate`].

**Protocol Additions — Tools & Data Access:**
- `datasets/...` and `facts/te_tag_map.csv` when semantics are uncertain.
...
- In Round 2, test and refute or support claims from DS/ME with data checks.

### 6. Boundaries, Anti-Goals & No-Gos
- No exclusive data hoarding.
...
- Dataset manifests + semantic map updates.
...
- Engage DS/ME claims; publish checks and references.
...
- `data_gap` → state required data and acquisition cost on the blackboard.

### 10. Metrics & Documentation
- Track references to your artifacts (reuse) and orphan rate.
